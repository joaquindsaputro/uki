...
let projectState = {
    topic: "",
    era: "",
    structure: "Linear", // Tambahan State Baru
...
function renderWritersRoom() {
    ...
    if (projectState.intro) {
        ...
        const rawIntroPrompt = projectState.lang === "English" 
            ? `Write an atmospheric, chilling, and highly engaging HOOK/INTRO narration (60-80 words) in ENGLISH. Position the listener as a witness. Based on this premise: "${projectState.intro.scene_premise}". Context: Documentary about "${projectState.topic}" set in ${projectState.era}. Output ONLY the narration text. NO conversational filler.`
            : `Tuliskan narasi HOOK/INTRO yang sangat mengikat, kelam, dan atmosferik (60-80 kata) dalam BAHASA INDONESIA. Posisikan pendengar sebagai saksi. Berdasarkan premise: "${projectState.intro.scene_premise}". Konteks: Dokumenter tentang "${projectState.topic}" di era ${projectState.era}. HANYA berikan teks narasi. TANPA penjelasan tambahan.`;
    ...
    projectState.acts.forEach((act, aIndex) => {
        ...
        act.scenes.forEach((scene, sIndex) => {
            ...
            const rawPromptText = projectState.lang === "English"
                ? `Write a voice-over narration of 70-90 words in ENGLISH for this scene: "${scene.scene_premise}". Context: A documentary about "${projectState.topic}" set in ${projectState.era}. Ensure the narration connects logically to the main story. Output ONLY the narration text. NO conversational filler.`
                : `Tuliskan narasi/Voice Over sepanjang 70-90 kata dalam BAHASA INDONESIA untuk adegan ini: "${scene.scene_premise}". Konteks: Dokumenter tentang "${projectState.topic}" di era ${projectState.era}. Jaga kontinuitas cerita agar terhubung logis. HANYA berikan teks narasi. TANPA penjelasan tambahan.`;
